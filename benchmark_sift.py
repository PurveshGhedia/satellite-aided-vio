"""
SIFT Matcher Benchmark on UAV-VisLoc Dataset
---------------------------------------------
Tests SIFT + Lowe's ratio test + RANSAC homography matcher against
real drone-to-satellite image pairs with GPS ground truth.

Key differences from ORB benchmark:
  - SIFT descriptors: scale and rotation invariant, float descriptors
  - FLANN matcher: faster for float descriptors than brute force
  - Lowe's ratio test: filters ambiguous matches (ratio < 0.75)
    instead of taking top N% by distance

Usage:
  python benchmark_sift.py --dataset /path/to/UAV_VisLoc_dataset --scene 03
  python benchmark_sift.py --dataset /path/to/UAV_VisLoc_dataset --scene all
"""
...
# ---------------------------------------------------------------------------
# Matching core — SIFT + FLANN + Lowe's ratio test + RANSAC
# ---------------------------------------------------------------------------

def sift_match(img_satellite, img_drone, ratio_thresh=0.75, ransac_thresh=5.0):
    """
    Match drone image against satellite patch using SIFT + FLANN + RANSAC.

    Key differences from ORB:
      - SIFT gives float descriptors that are more discriminative
        across scale and moderate viewpoint changes
      - FLANN is faster than brute force for float descriptors
      - Lowe's ratio test filters ambiguous matches by comparing
        the best match distance to the second-best match distance.
        A match is kept only if best_dist < ratio_thresh * second_best_dist.
        This removes matches where the descriptor could plausibly
        correspond to multiple keypoints (i.e. wrong matches).

    Args:
        img_satellite:  grayscale satellite patch
        img_drone:      grayscale drone image
        ratio_thresh:   Lowe's ratio test threshold (0.75 is standard)
    ...
    kp_sat,   des_sat = sift.detectAndCompute(img_satellite, None)
    kp_drone, des_drone = sift.detectAndCompute(img_drone,     None)
    ...
    # knnMatch returns 2 nearest neighbours per descriptor
    raw_matches = flann.knnMatch(des_drone, des_sat, k=2)
    ...
    if len(good) < 4:
        return {"success": False, "reason": "too few matches after ratio test",
...
def benchmark_scene(scene_dir, max_images=None, search_radius_px=600, verbose=True, rectify=False, hfov_deg=84.0):
    ...
    with rasterio.open(sat_path) as sat_ds:
        for img_path in drone_images:
            ...
            # Run matcher
            match_result = sift_match(sat_patch, drone_img, 0.85)
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark SIFT matcher on UAV-VisLoc")
    ...
    parser.add_argument("--output",       default="sift_benchmark_results.csv")
